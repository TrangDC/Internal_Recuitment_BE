...
type SkillRepository interface {
	...
	// common function
	ValidName(ctx context.Context, skillId uuid.UUID, name string) error
}
...
func (rps skillRepoImpl) BuildCreate() *ent.SkillCreate {
	return rps.client.Skill.Create().SetUpdatedAt(currentTime).SetCreatedAt(currentTime)
}

func (rps skillRepoImpl) BuildBulkCreate(ctx context.Context, input []*ent.SkillCreate) ([]*ent.Skill, error) {
	return rps.client.Skill.CreateBulk(input...).Save(ctx)
}

func (rps skillRepoImpl) BuildUpdate() *ent.SkillUpdate {
	return rps.client.Skill.Update().SetUpdatedAt(currentTime)
}

func (rps skillRepoImpl) BuildDelete() *ent.SkillUpdate {
	return rps.client.Skill.Update().SetDeletedAt(currentTime)
}
...
func (rps *skillRepoImpl) BuildUpdateOne(ctx context.Context, model *ent.Skill) *ent.SkillUpdateOne {
	return model.Update().SetUpdatedAt(currentTime)
}
...
func (rps *skillRepoImpl) DeleteSkill(ctx context.Context, record *ent.Skill) error {
	_, err := rps.BuildUpdateOne(ctx, record).SetDeletedAt(currentTime).Save(ctx)
	return err
}
...
// common function
func (rps *skillRepoImpl) ValidName(ctx context.Context, skillId uuid.UUID, name string) error {
	...
	if isExist {
		return fmt.Errorf("model.skills.validation.name_exist")
	}
	return err
}
