...
func (svc *skillTypeSvcImpl) CreateSkillType(ctx context.Context, input ent.NewSkillTypeInput, note string) (*ent.SkillTypeResponse, error) {
	...
	_, err = svc.repoRegistry.AuditTrail().AuditTrailMutation(ctx, result.ID, audittrail.ModuleSkillTypes, jsonString, audittrail.ActionTypeCreate, note)
	if err != nil {
		svc.logger.Error(err.Error(), zap.Error(err))
	}
...
func (svc *skillTypeSvcImpl) UpdateSkillType(ctx context.Context, skillTypeId uuid.UUID, input ent.UpdateSkillTypeInput, note string) (*ent.SkillTypeResponse, error) {
	...
	_, err = svc.repoRegistry.AuditTrail().AuditTrailMutation(ctx, skillTypeId, audittrail.ModuleSkillTypes, jsonString, audittrail.ActionTypeUpdate, note)
	if err != nil {
		svc.logger.Error(err.Error(), zap.Error(err))
	}
	return &ent.SkillTypeResponse{
...
func (svc *skillTypeSvcImpl) DeleteSkillType(ctx context.Context, skillTypeId uuid.UUID, note string) error {
	...
	_, err = svc.repoRegistry.AuditTrail().AuditTrailMutation(ctx, skillTypeId, audittrail.ModuleSkillTypes, jsonString, audittrail.ActionTypeDelete, note)
	if err != nil {
		svc.logger.Error(err.Error(), zap.Error(err))
	}
	return nil
