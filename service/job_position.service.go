...
func (svc *jobPositionSvcImpl) CreateJobPosition(ctx context.Context, input ent.NewJobPositionInput, note string) (*ent.JobPositionResponse, error) {
	...
	_, err = svc.repoRegistry.AuditTrail().AuditTrailMutation(ctx, result.ID, audittrail.ModuleJobPositions, jsonString, audittrail.ActionTypeCreate, note)
	if err != nil {
		svc.logger.Error(err.Error(), zap.Error(err))
	}
...
func (svc *jobPositionSvcImpl) UpdateJobPosition(ctx context.Context, jobPositionId uuid.UUID, input ent.UpdateJobPositionInput, note string) (*ent.JobPositionResponse, error) {
	...
	_, err = svc.repoRegistry.AuditTrail().AuditTrailMutation(ctx, jobPositionId, audittrail.ModuleJobPositions, jsonString, audittrail.ActionTypeUpdate, note)
	if err != nil {
		svc.logger.Error(err.Error(), zap.Error(err))
	}
	return &ent.JobPositionResponse{
...
func (svc *jobPositionSvcImpl) DeleteJobPosition(ctx context.Context, jobPositionId uuid.UUID, note string) error {
	...
	_, err = svc.repoRegistry.AuditTrail().AuditTrailMutation(ctx, jobPositionId, audittrail.ModuleSkillTypes, jsonString, audittrail.ActionTypeDelete, note)
	if err != nil {
		svc.logger.Error(err.Error(), zap.Error(err))
	}
	return nil
