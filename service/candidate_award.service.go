...
func (svc candidateAwardSvcImpl) ProcessCandidateAwardInput(ctx context.Context, candidateId uuid.UUID, input []*ent.CandidateAwardInput,
	...
	// Create new
	if len(newRecord) > 0 {
		createAttachments, err := repoRegistry.CandidateAward().BuildBulkCreate(ctx, newRecord, candidateId)
		if err != nil {
			return err
		}
	...
	if len(updateRecords) > 0 {
		createAttachments, err := repoRegistry.CandidateAward().BuildBulkUpdate(ctx, updateRecords)
		if err != nil {
			return err
		}
		attachments = append(attachments, createAttachments...)
