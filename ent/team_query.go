...
type TeamQuery struct {
	...
	withNamedMemberEdges  map[string]*UserQuery
...
func (tq *TeamQuery) QueryUserEdges() *UserQuery {
...
// QueryMemberEdges chains the current query on the "member_edges" edge.
func (tq *TeamQuery) QueryMemberEdges() *UserQuery {
	query := &UserQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (tq *TeamQuery) QueryUserTeams() *TeamManagerQuery {
	query := &TeamManagerQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TeamQuery) Clone() *TeamQuery {
	...
	return &TeamQuery{
		...
		withMemberEdges:  tq.withMemberEdges.Clone(),
...
func (tq *TeamQuery) WithUserEdges(opts ...func(*UserQuery)) *TeamQuery {
	query := &UserQuery{config: tq.config}
...
// WithMemberEdges tells the query-builder to eager-load the nodes that are connected to
// the "member_edges" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TeamQuery) WithMemberEdges(opts ...func(*UserQuery)) *TeamQuery {
	query := &UserQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withMemberEdges = query
	return tq
}
...
func (tq *TeamQuery) WithUserTeams(opts ...func(*TeamManagerQuery)) *TeamQuery {
	query := &TeamManagerQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withUserTeams = query
	return tq
}
...
func (tq *TeamQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Team, error) {
	...
	if query := tq.withUserEdges; query != nil {
		if err := tq.loadUserEdges(ctx, query, nodes,
			func(n *Team) { n.Edges.UserEdges = []*User{} },
			func(n *Team, e *User) { n.Edges.UserEdges = append(n.Edges.UserEdges, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := tq.withMemberEdges; query != nil {
		if err := tq.loadMemberEdges(ctx, query, nodes,
			func(n *Team) { n.Edges.MemberEdges = []*User{} },
			func(n *Team, e *User) { n.Edges.MemberEdges = append(n.Edges.MemberEdges, e) }); err != nil {
			return nil, err
		}
	}
	if query := tq.withUserTeams; query != nil {
		if err := tq.loadUserTeams(ctx, query, nodes,
			...
			func(n *Team, e *TeamManager) { n.Edges.UserTeams = append(n.Edges.UserTeams, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range tq.withNamedUserEdges {
		if err := tq.loadUserEdges(ctx, query, nodes,
	...
	for name, query := range tq.withNamedMemberEdges {
		if err := tq.loadMemberEdges(ctx, query, nodes,
			func(n *Team) { n.appendNamedMemberEdges(name) },
			func(n *Team, e *User) { n.appendNamedMemberEdges(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range tq.withNamedUserTeams {
		if err := tq.loadUserTeams(ctx, query, nodes,
			func(n *Team) { n.appendNamedUserTeams(name) },
			func(n *Team, e *TeamManager) { n.appendNamedUserTeams(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (tq *TeamQuery) loadUserEdges(ctx context.Context, query *UserQuery, nodes []*Team, init func(*Team), assign func(*Team, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(team.UserEdgesTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(team.UserEdgesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(team.UserEdgesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(team.UserEdgesPrimaryKey[1]))
...
func (tq *TeamQuery) loadMemberEdges(ctx context.Context, query *UserQuery, nodes []*Team, init func(*Team), assign func(*Team, *User)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Team)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(team.MemberEdgesColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (tq *TeamQuery) loadUserTeams(ctx context.Context, query *TeamManagerQuery, nodes []*Team, init func(*Team), assign func(*Team, *TeamManager)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Team)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.TeamManager(func(s *sql.Selector) {
		s.Where(sql.InValues(team.UserTeamsColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (tq *TeamQuery) WithNamedUserEdges(name string, opts ...func(*UserQuery)) *TeamQuery {
	query := &UserQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	if tq.withNamedUserEdges == nil {
...
// WithNamedMemberEdges tells the query-builder to eager-load the nodes that are connected to the "member_edges"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (tq *TeamQuery) WithNamedMemberEdges(name string, opts ...func(*UserQuery)) *TeamQuery {
	query := &UserQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	if tq.withNamedMemberEdges == nil {
		tq.withNamedMemberEdges = make(map[string]*UserQuery)
	}
	tq.withNamedMemberEdges[name] = query
	return tq
}

// WithNamedUserTeams tells the query-builder to eager-load the nodes that are connected to the "user_teams"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (tq *TeamQuery) WithNamedUserTeams(name string, opts ...func(*TeamManagerQuery)) *TeamQuery {
	query := &TeamManagerQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	if tq.withNamedUserTeams == nil {
		tq.withNamedUserTeams = make(map[string]*TeamManagerQuery)
	}
	tq.withNamedUserTeams[name] = query
	return tq
}
